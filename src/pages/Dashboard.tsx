...
export default function Dashboard() {
    ...
    const fetchFinancials = async () => {
        ...
        const realizedOrders = allOrders?.filter(o => ['entregado', 'terminado'].includes(o.status)) || []
                            ...
                            <div className="space-y-4">
                                {history.map((item) => (
                                    <div key={item.id} className="flex justify-between items-center border-b pb-2 last:border-0 last:pb-0">
                                        ...
                                        <div className={`font-bold text-sm ${item.status === 'cancelado' ? 'text-slate-300 line-through' : item.type === 'income' ? 'text-green-600' : 'text-slate-900'}`}>
